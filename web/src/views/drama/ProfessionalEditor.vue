            ...
            <div v-if="currentStoryboard" class="shot-editor-new">
              ...
              <div class="scene-section">
                <div class="section-label">
                  ...
                  <el-button size="small" text @click="showSceneSelector = true">{{
              ...
              <div class="cast-section">
                ...
                <div class="cast-list">
                  ...
                  <div
                    v-if="!currentStoryboard?.characters || currentStoryboard.characters.length === 0"
                    class="cast-empty"
                  >
              ...
              <div class="cast-section">
                <div class="section-label">
                  ...
                  <el-button size="small" text :icon="Plus" @click="showPropSelector = true">{{
                    $t('editor.addProp')
                  }}</el-button>
                </div>
                <div class="cast-list">
                  ...
                  <div v-if="!currentStoryboardProps || currentStoryboardProps.length === 0" class="cast-empty">
                    {{ $t('editor.noProps') }}
                  </div>
                </div>
              </div>
            ...
            <div class="tab-content" v-if="currentStoryboard">
              <div class="image-generation-section">
                ...
                <div class="frame-type-selector">
                  ...
                  />
                  <span v-if="selectedFrameType === 'panel'" class="panel-count-label">{{
                    $t('editor.panelCount')
                  }}</span>
                </div>
                ...
                <!-- 生成结果 -->
                <div class="generation-result" v-if="generatedImages.length > 0 || selectedFrameType === 'action'">
                  ...
                  <div class="image-grid">
                    ...
                    <div v-for="img in generatedImages" :key="img.id" class="image-item-wrapper">
                      <div
                        class="image-item"
                        :class="{
                      ...
                      >
                        ...
                        <div class="image-actions" v-if="hasImage(img)">
                          ...
                          <div v-else></div>
                          <!-- 删除按钮 -->
                          <div v-if="hasImage(img)" class="delete-icon-overlay" @click.stop="handleDeleteImage(img)">
          ...
          <el-tab-pane :label="$t('video.videoGeneration')" name="video">
            <div class="tab-content" v-if="currentStoryboard">
              <div class="video-generation-section">
                  ...
                  <div class="param-row">
                    ...
                    <div style="flex: 1; display: flex; align-items: center">
                      ...
                      <span style="margin-left: 10px; min-width: 40px"
                        >{{ videoDuration }}{{ $t('professionalEditor.seconds') }}</span
                      >
                    </div>
                  </div>
                </div>
                ...
                <!-- 选择参考图片 -->
                <div
                  v-if="selectedReferenceMode && selectedReferenceMode !== 'none'"
                  ...
                  <div class="frame-type-content">
                      ...
                      <div v-if="previousStoryboardLastFrames.length > 0" class="previous-frame-section">
                        <div style="display: flex; align-items: center; gap: 6px; margin-bottom: 6px">
                          <el-tag size="small" type="primary">
                            上一镜头 #{{ previousStoryboard?.storyboard_number }}
                            尾帧
                          </el-tag>
                      ...
                      >
                        <div
                          v-for="img in videoReferenceImages.filter(
                            (i) => i.status === 'completed' && i.image_url && i.frame_type === 'first',
                          )"
                          :key="img.id"
                          class="reference-item"
                          :class="{
                    ...
                    >
                      ...
                      >
                        <div
                          v-for="img in videoReferenceImages.filter(
                            (i) => i.status === 'completed' && i.image_url && i.frame_type === 'key',
                          )"
                          :key="img.id"
                          class="reference-item"
                          :class="{
                    ...
                    >
                      ...
                      >
                        <div
                          v-for="img in videoReferenceImages.filter(
                            (i) => i.status === 'completed' && i.image_url && i.frame_type === 'last',
                          )"
                          :key="img.id"
                          class="reference-item"
                          :class="{
                    ...
                    >
                      ...
                      >
                        <div
                          v-for="img in videoReferenceImages.filter(
                            (i) => i.status === 'completed' && i.image_url && i.frame_type === 'panel',
                          )"
                          :key="img.id"
                          class="reference-item"
                          :class="{
                  ...
                  <div class="image-slots-container" style="margin-top: 16px; margin-bottom: 24px">
                    ...
                    <div v-else-if="selectedReferenceMode === 'multiple'" style="text-align: center">
                      ...
                      <div style="display: flex; gap: 12px; justify-content: center; flex-wrap: wrap">
                        <div
                          v-for="index in currentModelCapability?.maxImages || 6"
                          :key="index"
                          ...
                          "
                          @click="
                            selectedImageObjects[index - 1] && removeSelectedImage(selectedImageObjects[index - 1].id)
                          "
                        >
                          <img
                ...
                <div class="generation-controls" style="margin-top: 32px; text-align: center">
                  <el-button
                    ...
                    :disabled="
                      !selectedVideoModel || (selectedReferenceMode !== 'none' && selectedImagesForVideo.length === 0)
                    "
                    @click="generateVideo"
                  >
                ...
                <div class="generation-result" v-if="generatedVideos.length > 0" style="margin-top: 24px">
                  ...
                  <div class="image-grid">
                    <div v-for="video in generatedVideos" :key="video.id" class="image-item-wrapper">
                      <div class="image-item video-item">
                        <div v-if="video.video_url" class="video-thumbnail" @click="playVideo(video)">
                        ...
                        <div class="video-actions">
                          ...
                          >
                            <el-icon :size="18" color="var(--text-primary)" v-if="!addingToAssets.has(video.id)">
                              <FolderAdd />
                            </el-icon>
                            <el-icon :size="18" color="var(--text-primary)" v-else class="is-loading">
                              <Loading />
                            </el-icon>
                          </div>
                          <div v-else></div>
                          <!-- 删除按钮 -->
                          <div class="delete-video-button" @click.stop="handleDeleteVideo(video)">
                            <el-icon :size="18" color="red">
                              <DeleteFilled />
                            </el-icon>
                          </div>
                  ...
                  >
                    ...
                    <div class="merge-content">
                      ...
                      <div class="merge-header">
                        ...
                        <el-tag
                          :type="
                            merge.status === 'completed' ? 'success' : merge.status === 'failed' ? 'danger' : 'warning'
                          "
                          effect="dark"
                          size="large"
                          round
                      ...
                      <div class="merge-actions">
                        <template v-if="merge.status === 'completed' && merge.merged_url">
                          <el-button
                            type="primary"
                            :icon="VideoCamera"
                            @click="downloadVideo(merge.merged_url, merge.title)"
                            round
                          >
                            下载视频
                          </el-button>
                        ...
                        <el-button type="danger" :icon="Delete" @click="deleteMerge(merge.id)" round> 删除 </el-button>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <div style="display: flex; justify-content: space-between; align-items: center">
            <div>
              ...
              <span v-if="previewVideo.duration" style="margin-left: 12px; color: #606266; font-size: 14px"
                >{{ $t('professionalEditor.duration') }}: {{ previewVideo.duration
...
const extractFramePrompt = async () => {
  ...
  try {
    ...
    if (result.single_frame) {
      ...
      // 多帧情况，将所有帧的prompt合并
      extractedPrompt = result.multi_frame.frames.map((frame: any) => frame.prompt).join('\n\n')
    }
...
// 检查是否正在生成提示词
const isGeneratingPrompt = (storyboardId: number | undefined, frameType: string) => {
...
// 加载分镜的图片列表
const loadStoryboardImages = async (storyboardId: string | number, frameType?: string) => {
...
const generateFrameImage = async () => {
  ...
  try {
    ...
    // 提示信息
    const refMsg = referenceImages.length > 0 ? ` (已添加${referenceImages.length}张参考图)` : ''
...
const addVideoToAssets = async (video: VideoGeneration) => {
  ...
  try {
    ...
    if (video.storyboard_id) {
      const existingAsset = videoAssets.value.find((asset: any) => asset.storyboard_id === video.storyboard_id)
    ...
    if (isReplacing && video.storyboard_id && video.video_url) {
      ...
      if (timelineEditorRef.value) {
        timelineEditorRef.value.updateClipsByStoryboardId(video.storyboard_id, video.video_url)
      } else {
...
const handleDeleteVideo = async (video: VideoGeneration) => {
  ...
  try {
    await ElMessageBox.confirm('确定要删除这个视频吗？删除后无法恢复。', '确认删除', {
...
const lastFrameSlotImage = computed(() => {
  ...
  // 同时搜索当前镜头图片和上一镜头尾帧
  return (
    videoReferenceImages.value.find((img) => img.id === selectedLastImageForVideo.value) ||
...
const generateVideo = async () => {
  ...
  if (selectedReferenceMode.value !== 'none' && selectedImagesForVideo.value.length > 0) {
    // 同时搜索当前镜头图片和上一镜头尾帧
    selectedImage =
      videoReferenceImages.value.find((img) => img.id === selectedImagesForVideo.value[0]) ||
...
const toggleCharacterInShot = async (charId: number) => {
  ...
  // 保存到后端
  try {
    const characterIds = currentStoryboard.value.characters.map((c) => (typeof c === 'object' ? c.id : c))
...
const handleAddStoryboard = async () => {
  ...
  try {
    const nextShotNumber =
      storyboards.value.length > 0 ? Math.max(...storyboards.value.map((s) => s.storyboard_number)) + 1 : 1
...
const handleDeleteStoryboard = async (storyboard: any) => {
  try {
    await ElMessageBox.confirm(`确定要删除镜头 ${storyboard.storyboard_number} 吗？此操作不可恢复。`, '删除确认', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning',
    })
...
const deleteMerge = async (mergeId: number) => {
  try {
    await ElMessageBox.confirm('确定要删除此合成记录吗？此操作不可恢复。', '删除确认', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning',
    })
